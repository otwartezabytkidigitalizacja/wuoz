...
<div class="login-cnt <?php echo is_user_logged_in() ? 'logged': ''; ?> large-4 columns">
  <?php if ( is_user_logged_in() ) : ?>
  <p class="ta-right"><?php _e('witaj, ', 'otwarte2013') . '<strong>' ?><?php echo $current_user->user_login . '</strong>' ?>
    <span> | </span>
    <a href="<?php echo wp_logout_url(home_url()); ?>"> <?php _e('wyloguj', 'otwarte2013')?> </a>
  </p>
  <a data-reveal-id="settings-reveal" class="oz-button right"> <?php _e('Ustawienia', 'otwarte2013');?><span class="ico set-ico right"> </span> </a>
  <a href="<?php echo get_permalink(get_option('oz_my_catalogues_page'))?>" class="oz-button right first-btn"> <?php _e('Moje katalogi', 'otwarte2013');?><span class="ico mycats-ico right"> </span>  </a>
...
<div id="login-popup" class="reveal-modal small" data-options="closeOnBackgroundClick:false">
  <?php if (!empty($error) && $_POST['action'] == 'log-in') : ?>
  <?php $msgs = $error->get_error_messages();
  foreach ($msgs as $msg ) {
...
<form name="loginform" id="loginform" method="post">
  <p class="login-username">
    <input type="text" required placeholder="<?php _e('email lub nazwa użytkownika', 'otwarte2013')?>" name="log" id="user_login" class="input" size="20" value="<?=isset($_POST['log']) ? $_POST['log'] : '' ?>">
  ...
  <p style="text-align: center;">
    ...
    |
    <a href="<?php echo wp_lostpassword_url(); ?>"><?php _e('nie pamiętam hasła', 'otwarte2013'); ?></a>
  </p>
</form>
