(function ($) {
  ...
  init_home_map = function () {
    ...
    //alert($('#home-map').gmap3({get : {name:"clusterer"}}));
  };
  ...
  get_map_objects = function(paged, total) {
    ...
    var data = {action: 'map_objects', paged: paged};
    var table = [];
    $.ajax({
      type : 'POST',
      ...
      success : function(response) {
        ...
        if(paged===1) {
                  ...
                  click: function (cluster, event, context) {
                    ...
                    if(m.getZoom()<12) {
                      ...
                      if (markers_length <= 55) {
                        ...
                        var monument_data = {action: 'monument_data_multiple', ids: ids};
                        ...
                        $.post(otwarte_data.ajax_url, monument_data, function (response) {
                          ...
                          $('#home-map').gmap3({clear: "overlay"}, {
                            overlay: {
                              latLng: cluster.main.getPosition(), options: {
                                content:
                                '<div id="ovr' + context.id + '" class="map-ovr">' + '<h4 class="label">' + 'Lista zabytków w tym miejscu' + '</h4>' +
                                '<div class="ovr-content">' + text + '</div></div>', offset: {x: 20, y: -54}
                              }
                            }
                          });
                        });
                      ...
                      $('#home-map').gmap3({clear: "overlay"}, {
                        overlay: {
                          latLng: marker.getPosition(), options: {
                            ...
                            '<div class="ovr-content">' + data.excerpt + '</div>' + '<div class="ovr-button-container"><a href="'+ data.permalink +'"></a></div>' + '</div>', offset: {x: 20, y: -54},
                          }
                        }
                      });
                    });
          ...
          var cls = $('#home-map').gmap3({get:{name:"clusterer", all:true}});
          $('#home-map').gmap3({
            marker : {
              values: table,
              cluster : cls[0],
  ...
  thumb_events = function () {
    ...
    $('.single-thumb[data-post-id]').on({click: function(){
                              ...
                              $('.single-cnt .info-button').on(
                                ...
                                get_single_object = function (e) {
                                  ...
                                  $.post(otwarte_data.ajax_url, data, function (response) {
                                    ...
                                    $('a.close-popup.close-reveal-modal').on({click: function(){
                                              ...
                                              $('#nav-prev-page').on(
                                                ...
                                                get_single_object_inside = function (e) {
                                                  ...
                                                  $.post(otwarte_data.ajax_url, data, function (response) {//console.info(response);
                                                    ...
                                                    $('#inside-controler a.close-popup').on({click: function(){
                                                      var title;
                                                    ...
                                                    $('.single-cnt img').last().load(function () {
                                                      ...
                                                      $('#zoomInButton').on(
                                                        ...
                                                        insidePopupThumbEvents = function () {
                                                          ...
                                                          $('#related-docs .single-thumb[data-post-id]').on({click: function(){
                                                            var href = $('a', this).attr('href');
                                                            var title;
