...
function otwarte_map_objects() {
	$a = array('post_type' => 'monument', 'posts_per_page' => 2000); 
	$q = new WP_Query($a);
	$i=0;
	while($q->have_posts()) {
		$q->the_post();
		$lat = get_post_meta($q->post->ID, 'oz_lat', true );
		$lng = get_post_meta($q->post->ID, 'oz_lng', true );
		if($lat!='' && $lng!='') {
			$posts[$i]['lat']=get_post_meta($q->post->ID, 'oz_lat', true );
			$posts[$i]['lng']=get_post_meta($q->post->ID, 'oz_lng', true );
			$posts[$i]['id']=$q->post->ID;
			$i++;
		}
	}
